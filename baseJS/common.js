...
var filePath = 'http://yun.visitxm.com/';
var baseApi = "http://jpn.visitxm.com/api";
...
var utils = {
    compactQueue: function compactQueue(queue) {
        var obj;
    ...
    generateArrayPrefix: function generateArrayPrefix(prefix, key) {
...
var stringify = function stringify(object, options) {
    ...
    var prefix = option.prefix,
        generateArrayPrefix = option.generateArrayPrefix,
        strictNullHandling = option.strictNullHandling,
        skipNulls = option.skipNulls,
        encoder = option.encoder,
        filter = option.filter,
        sort = option.sort,
        allowDots = option.allowDots,
        serializeDate = option.serializeDate,
        formatter = option.formatter,
        encodeValuesOnly = option.encodeValuesOnly;
    ...
    for (var i = 0; i < objKeys.length; ++i) {
        ...
        if (Array.isArray(obj)) {
            values = values.concat(this.stringify(obj[key], { prefix: generateArrayPrefix(prefix, key),
            ...
            values = values.concat(this.stringify(obj[key], { prefix: prefix + (allowDots ? '.' + key : '[' + key + ']'),
                generateArrayPrefix: generateArrayPrefix,
                strictNullHandling: strictNullHandling,
                skipNulls: skipNulls,
                encoder: encoder,
...
xm = xmlyw = {
     ...
     * @returns {null}
     */
    formatDate: function formatDate(val, pattern) {
        if (!val) {
            return null;
        }
        if (!pattern) {
